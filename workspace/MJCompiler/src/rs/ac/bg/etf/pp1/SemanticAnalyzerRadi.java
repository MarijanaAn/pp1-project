...
public class SemanticAnalyzerRadi extends VisitorAdaptor {
...
//-----------------------------------varDecls---------------------------------------------
	...
	public void visit(VarDeclName var) {
		...
		if(variable == Tab.noObj  || variable== null) {
			if(curNamespace == null || (curNamespace!=null && curMethod!=null)) variable = Tab.insert(Obj.Var, var.getVarName(), curType);	
	...
	public void visit(VarDeclArray var) {
		...
		if(variable == Tab.noObj  || variable== null) {
			if(curNamespace == null ||(curNamespace!=null && curMethod!=null)) variable = Tab.insert(Obj.Var, var.getVarName(), new Struct(Struct.Array, curType));
...
//-----------------------------------methods-------------------------------------------------
	...
	public void visit(MethName mn) {
		if(curNamespace!=null) curMethod = Tab.insert(Obj.Meth, curNamespace.getName()+ "::"+ mn.getMethName(), curType);
		else curMethod = Tab.insert(Obj.Meth, mn.getMethName(), curType);
		mn.obj = curMethod;
		//report_info(""+curMethod.getName(), mn);
		Tab.openScope();
	...
	public void visit(MethodVoidName mn) {
		if(curNamespace!=null) curMethod = Tab.insert(Obj.Meth, curNamespace.getName()+ "::"+ mn.getMethName(), curType);
		else curMethod = Tab.insert(Obj.Meth, mn.getMethName(), Tab.noType);
		mn.obj = curMethod;
		Tab.openScope();
...
//		Obj desgn = null;
//		if(curNamespace != null  && curMethod==null) desgn = Tab.find(curNamespace.getName() + "::" + dn.getI1());
//		else desgn = Tab.find(dn.getI1());
		Obj desgn = null;
		desgn = Tab.find(dn.getI1());
		if(desgn == Tab.noObj) desgn = Tab.find(curNamespace.getName() + "::" + dn.getI1());
		dn.obj = desgn;
		if(desgn == Tab.noObj) {
...
//		Obj desgn = null;
//		if(curNamespace != null && curMethod==null ) desgn = Tab.find(curNamespace.getName() + "::" + d.getName());
//		else desgn = Tab.find(d.getName());
		Obj desgn = null;
		if(curMethod !=null)
		desgn = Tab.find(d.getName());
		if(desgn == Tab.noObj) {desgn = Tab.find(curNamespace.getName() + "::" + d.getName());
		report_info("" + curNamespace.getName(), d);}
		if(desgn == Tab.noObj) {
	...
	public void visit(DesignatorArray d) {
		...
		if(expr !=Tab.intType) {
				...
				report_info("Koristi se element niza " + arr.getName(), d);
			}
		//}
	}
	...
	public void visit(DesignatorScope ds) {
		...
		if(desgn == Tab.noObj) {
			...
			if(desgn.getKind() == Obj.Var) {
				report_info("Nadjena promenljiva " + desgn.getName() +": " + TypeToString(desgn.getType().getKind()) + ", " + 
			desgn.getAdr() + ", " + desgn.getLevel() , ds);
			}
			else if(desgn.getKind() == Obj.Con) report_info("Nadjena konstanta " + desgn.getName() +": " + TypeToString(desgn.getType().getKind()) + ", " + 
					desgn.getAdr() + ", " + desgn.getLevel() , ds);
			else if(desgn.getKind() == Obj.Meth) report_info("Nadjen poziv metode " + desgn.getName() +": " + TypeToString(desgn.getType().getKind()) + ", " + 
	...
	if(expr !=Tab.intType) {
				...
				report_info("Koristi se element niza " + arr.getName() , das);
...
//	private int	forLoop=0;
	...
	public void visit(Break br) { 
		if(!inFor) report_error("Break naredba se sme naci samo unutar for petlje!", br);
	}
	
	public void visit(Continue cnt) {  
		if(!inFor) report_error("Continue naredba se sme naci samo unutar for petlje!", cnt);
	}
